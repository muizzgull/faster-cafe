const products = [
    ...
    { id: 10, name: "Faster Special Pizza", price: 1090, image: "faster-special-pizza.jpeg", category: "Pizza" },
    { id: 11, name: "Chicken Mexican Pizza", price: 1090, image: "chicken-mexican-pizza.jpeg", category: "Pizza" },
    { id: 12, name: "Hot & Spicy Pizza", price: 1090, image: "hot-n-spicy-pizza.jpeg", category: "Pizza" },
    { id: 13, name: "Super Supreme Pizza", price: 1090, image: "super-supreme-pizza.jpeg", category: "Pizza" },
    { id: 14, name: "BBQ Lover Pizza", price: 1090, image: "bbq-lover-pizza.jpeg", category: "Pizza" },
    { id: 15, name: "Cheese Lover Pizza", price: 1090, image: "cheese-lover-pizza.jpeg", category: "Pizza" },
    ...
    { id: 2001, name: "Deal 1: <br> 1 Patty Burger + 1 Fries + Drink", price: 430, image: "deal1.jpeg", category: "Deals" },
    { id: 2002, name: "Deal 2: <br> 1 Zinger Burger + 1 Fries + Drink", price: 550, image: "deal2.jpeg", category: "Deals" },
    { id: 2003, name: "Deal 3: <br> 1 Grilled Burger + 1 Fries + Drink", price: 650, image: "deal3.jpeg", category: "Deals" },
    { id: 2004, name: "Deal 4: <br> 2 Zinger Burgers + 2 Fries + 2 Drinks", price: 990, image: "deal4.jpeg", category: "Deals" },
    { id: 2005, name: "Deal 5: <br> 2 Grilled Burgers + 2 Fries + 2 Drinks", price: 1150, image: "deal5.jpeg", category: "Deals" },
    { id: 2006, name: "Deal 6: <br> 4 Zinger Burgers + 1 Fries + 1.5L", price: 1850, image: "deal6.jpeg", category: "Deals" },
    ...
    { id: 3006, name: "Special Deal 6: <br> 1 Monster Fillet + Drink", price: 650, image: "special-deal6.PNG", category: "Special Deals" },
    ...
    { id: 4006, name: "Pizza Deal 6: <br> 1 Small Pizza + 1 Drink(NR)", price: 650, image: "pizza-deal6.jpeg", category: "Pizza Deals" },
...
function injectNavbar() {
    ...
    <header class="fixed top-0 left-0 w-full bg-white/90 backdrop-blur-md z-[1000] border-b border-gray-100 px-4 md:px-6 py-4">
        <div class="max-w-7xl mx-auto flex justify-between items-center">
            <a href="index.html"><img class="w-32 md:w-48" src="logo.png" alt="FASTER CAFE"></a>
        ...
        if (filtered.length > 0) {
            ...
            htmlContent += filtered.map(p => {
                ...
                <div class="bg-white p-4 rounded-[30px] shadow-sm border border-gray-100 flex flex-col justify-between">
                    ...
                    <div>
                        <h3 class="text-xs md:text-base font-black uppercase italic mb-1">${p.name}</h3>
                        <p class="text-gray-400 text-[8px] uppercase mb-2">${p.category}</p>
                        <p class="text-[#E72229] font-black text-sm">Rs. ${p.price}</p>
                        <button onclick="handleAddToCart(${p.id})" class="mt-3 w-full bg-black text-white py-3 rounded-xl font-black text-[10px] hover:bg-[#E72229] transition uppercase">
...
function handleAddToCart(productId) {
    ...
    const needsModal = ["Pizza", "Signature Pizza", "Hot Wings", "Nuggets", "Chicken Strips", "Pizza Deals"].includes(p.category) || (p.category === "Fries" && p.name !== "Loaded Fries");
    if (needsModal) openModal(p); else addToCart(productId);
}
...
function openModal(product) {
    ...
    document.getElementById('modal-pizza-name').innerHTML = product.name;
    ...
    else if (product.category === "Pizza Deals") { const pizzaFlavors = products.filter(p => p.category === "Pizza"); opt = pizzaFlavors.map((p, index) => renderOption(p.name, product.price, index === 0)).join(''); } 
    else if (product.category === "Hot Wings") { const isPremium = product.name.includes("Honey") || product.name.includes("Malai"); opt = `${renderOption('5 Pcs', isPremium ? 390 : 350, true)} ${renderOption('10 Pcs', isPremium ? 790 : 690)}`; } 
    else if (product.category === "Fries") { const isPizza = product.name === "Pizza Fries"; opt = `${renderOption('Regular', isPizza ? 490 : 250, true)} ${renderOption('Large', isPizza ? 790 : 350)}`; } 
    else if (product.category === "Nuggets") { opt = `${renderOption('5 Pieces', 300, true)} ${renderOption('10 Pieces', 590)}`; } 
    else if (product.category === "Chicken Strips") { opt = `${renderOption('5 Pieces', 420, true)} ${renderOption('10 Pieces', 790)}`; }
...
function closePizzaModal() { 
    ...
    setTimeout(() => { modal.classList.replace('flex', 'hidden'); toggleBodyScroll(false); }, 300); 
}

function renderOption(label, price, checked = false) {
    return `<label class="flex items-center justify-between p-3 bg-gray-50 rounded-xl cursor-pointer border-2 border-transparent has-[:checked]:border-[#E72229] transition-all"><div class="flex items-center gap-3"><input type="radio" name="item-choice" value="${label}" data-price="${price}" ${checked ? 'checked' : ''} class="w-4 h-4 accent-[#E72229]"><span class="font-bold text-xs uppercase">${label}</span></div><span class="font-black text-[#E72229] text-sm text-right">Rs. ${price}</span></label>`;
}
...
function addToCart(productId) {
    ...
    save(); showToast(`${p.name.replace('<br>', '')} Added!`);
}
